...
def load_snapshot(path: Path) -> SourceIndex:
    ...
    Needs neither the SWAT+ source nor ``swatplus_reference``. Note that
    ``scope_at``/``breakpoint`` still read the Fortran files themselves, so on a
    machine without the source those degrade to "unresolved" rather than
    returning something wrong -- every other query is fully served.
    """
    try:
    ...
    index = SourceIndex(provenance=Provenance(**payload["provenance"]))

    for raw in payload["procedures"]:
        index.procedures[raw["name"].lower()] = Procedure(**raw)
    ...
    index.call_paths = defaultdict(list, {
        k: [list(p) for p in v] for k, v in payload["call_paths"].items()
    })
